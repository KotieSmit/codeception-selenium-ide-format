...
function formatCommands(commands, indent = 0) {
    ...
    for (var i = 0; i < commands.length; i++) {
        ...
        if (command.command == 'assertConfirmation') {
            command.command = 'verifyConfirmation';
    ...
    if (command.type == 'command') {
        ...
        switch (command.command) {
            ...
            case 'clickAndWait':
                ...
                if (command.value == 'magentoAdminNavClick') {
                    var $script = 'return document.querySelector("' + target + '").click()';
            ...
            case 'assertText':
                if (command.target.substring(0, 4) === 'link') {
                    result += '->see("'+escapeString(command.value)+'");\n';
                } else {
                    var target = getSelector(command.target);
                    result += '->see("'+escapeString(command.value)+'", "'+target+'");\n';
                }
                break;
            case 'waitForVisible':
            ...
            case 'assertElementPresent':
                var target = getSelector(command.target);
                result += '->seeElement("'+target+'");\n';
                break;
            case 'waitForText':
            case 'waitForTextPresent':
                if (command.target) {
                    var target = getSelector(command.target);
                    result += '->waitForText("'+ escapeString(command.value)+'",'+10+ ' , "'+target+'");\n';
                } else {
                    result += '->waitForText("'+ escapeString(command.value)+'",'+10+ ');\n';
                }
                break;
            case 'assertNotText':
            case 'assertTextNotPresent':
                if (command.target) {
                    var target = getSelector(command.target);
                    result += '->dontSee("'+ escapeString(command.value)+'", "'+target+'");\n';
                } else {
                    result += '->dontSee("'+ escapeString(command.value)+'");\n';
                }
                break;
            case 'addSelection':
                var target = getSelector(command.target);
            ...
            case 'assertValue':
                var target = getSelector(command.target);
                result += '->seeInField("'+target+'", "'+ escapeString(command.value)+'");\n';
                break;
            ...
            case 'verifyConfirmation':
                result += '->seeInPopup("'+escapeString(command.target)+'");\n';
                break;
            case 'selectWindow':
                ...
                break;
            case 'verifyTitle':
                result += '->seeInTitle("'+command.target+'");\n';
                break;
            case 'deleteCookie':
                result += '->resetCookie("'+command.target+'");\n';
                break;
            case 'pause':
                //Must limit a pause to 10 seconds Max
                var waitTime = command.target / 1000; //convert milliseconds to seconds
                if (waitTime > 10) {
                    waitTime = 10; //Limit to 10 seconds
                }
                result += '->wait("'+waitTime+'");\n';
                break;
            case 'mouseOver':
                var target = getSelector(command.target);
                result += '->moveMouseOver("'+target+'");\n';
                break;
            case 'keyPress':
                var target = getSelector(command.target);
                result += '->pressKey("'+target+'","'+ command.value+'");\n';
                break;
            default:
